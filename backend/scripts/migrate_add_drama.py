"""
数据库迁移脚本 - 添加剧本生成模块相关表
创建: script_projects, script_nodes, script_sessions
"""
import asyncio
import sys
import os
...
DRAMA_TABLES = ["script_projects", "script_nodes", "script_sessions"]
...
async def migrate_add_drama():
    """创建剧本模块所需的三张新表（如果不存在）"""
    async with engine.begin() as conn:
        existing = []
        missing = []

        for table_name in DRAMA_TABLES:
