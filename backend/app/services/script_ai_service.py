...
【标准格式（必须严格遵守）】
每场固定四行起手：
  {episode_number}-1 场景名 时间（日/夜/晨/黄昏） 内/外
  人物：角色A、角色B
  △ 动作或场景描写（每行≤15字，≤5行）
  角色（情绪）：台词
...
符号说明：
  △ = 动作/场景描写行
  （OS）= 内心独白，写在角色名后：角色（OS）：内心独白
  VO = 画外音旁白：VO：旁白内容
  情绪括号可叠加：角色（隐忍→崩溃）：台词

每集输出 3-5 场，场次号连续递增：{episode_number}-1、{episode_number}-2……
...
禁止：大空镜、全景建立场景、缓慢环境描写、无动作的旁白开场

【对白规范】
- 绝大多数台词 < 15 字，情绪括号必填
- 每人连续对白 ≤ 3 句，中间穿插 △ 动作或对方反应
- 禁止书面语、解释性对白、说教性对白

【VO/OS规范】
- 每场 ≤ 2 条，仅用于关键转折或无法用动作表现的心理
- 禁止用 VO 交代剧情背景

【集末卡点（最后一行必须）】
最后一行必须是悬念/反转/危机/新人物/能力展示之一，驱动观众看下一集。
...
【爽点兑现规则】
- 每集至少1个明确爽点（打脸/复仇/逆袭/真相大白），当场兑现
- 禁止"下一集再报复"，结尾留悬念但本集主冲突必须有结果
