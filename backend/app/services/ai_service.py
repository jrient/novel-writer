...
# ---------------------------------------------------------------------------
# 质量指导（基于评审手册 v5 的 57 部剧本数据提炼）
# ---------------------------------------------------------------------------
QUALITY_GUIDANCE = """
【创作质量标准（7 维度）】
写作时请在每个维度上达到 7 分以上的水平：

1. 题材创新：在成熟套路骨架上做 1-2 个核心元素微创新（职业替换、反常规人设）。
   ✗ 不要纯粹"元素缝合"或照搬近期爆款换汤不换药。

2. 开局钩子：前三集（甚至第一集前三场戏）必须抛出核心卖点和强冲突，确立主角核心困境。
   ✗ 不要用大段背景旁白或平淡日常琐事开局。

3. 人设立体：赋予主角清晰的内在动机和外在目标，让角色主动破局。
   ✗ 不要塑造"工具人"反派，反派的恶必须有逻辑支撑。

4. 节奏冲突：每一集都要有明确的情绪起伏（卡点），"小高潮不断，大高潮期待"。
   ✗ 不要在非核心剧情上注水拖延节奏。

5. 文笔台词：精炼台词，每句对话体现人物性格、推动剧情或挑动情绪。
   ✗ 涉及专业领域时不要凭空想象，避免低级常识错误。

6. 爽点兑现：精心设计"期待-压抑-爆发"的爽点链路，压抑越狠、爆发越爽。
   ✗ 不要让主角一直憋屈，爽点兑现不能全靠运气或天降神兵。

7. 差异化：在动笔前明确对标竞品，用一句话总结你的剧本与竞品最大的不同点。
   ✗ 不要同质化严重、盲目跟风。
""".strip()
...
GENRE_GUIDANCE = {
    "萌宝": """
【萌宝类型专项要求】
- 萌宝属性必须打破"乖巧奶团"刻板印象，赋予反差属性（好赌、武力值高等）
- 开局极速绑定核心人物关系（父/母/子），男主不能出场太晚
- 成年主角不能沦为"带娃工具人"，女主必须有独立高光和主视角
- 情绪给足：极致虐心或极致搞笑，精准打中下沉市场
- ✗ 不要仅靠"下山找爹""带球跑"等经典套路无脑堆砌
- ✗ 不要为虐而虐导致逻辑硬伤（如强行制造不合理的惨况）
""".strip(),
    "女频": """
【女频类型专项要求】
- 男女主人设必须有鲜明标签和反差萌，CP 化学反应是核心
- 爽点兑现"快准狠"：女主面对挑衅必须"不服就干"，反击要物理+言语双重打击
- 利用信息差或金手指建立绝对优势，展现女性独立与主观能动性
- 情感线不能喧宾夺主挤压主线，要服务于角色成长
- ✗ 不要让女主陷入无意义内耗或圣母人设
- ✗ 不要频繁闪回打碎叙事节奏
""".strip(),
    "男频": """
【男频类型专项要求】
- 核心是升级体系和爽感节奏，金手指的规则感要清晰
- 打脸桥段要有铺垫压抑，不能一路碾压毫无悬念
- 配角体系要有记忆点，不能全是龙套式"经验包"
- 世界观架构要自洽，力量体系层次分明
- ✗ 不要让主角被系统或剧情推着走，要有主观能动性
- ✗ 不要让反派强行降智只为给主角送人头
""".strip(),
}


def build_quality_guidance(genre: str = "") -> str:
    """根据类型拼接通用+专项质量指导"""
    parts = [QUALITY_GUIDANCE]
    if genre:
        for key, guidance in GENRE_GUIDANCE.items():
            if key in genre:
...
请输出润色后的角色设定（保持原有的字段结构，直接输出内容即可）：""",
    ...
    "generate_title": """你是一位经验丰富的小说编辑。请根据以下章节内容，生成一个精炼、有吸引力的章节标题。

要求：
- 标题应简洁有力，一般4-10个字
- 能概括章节核心内容或情感基调
- 具有文学性和吸引力
- 只输出标题文字，不要加引号、书名号或其他符号
- 不要输出"第X章"等前缀
...
请生成章节标题：""",
    ...
    "plot_enhance": """你是一位经验丰富的小说作家和剧情策划。用户提供了一段剧情描述，请结合已有的前文内容、人物设定、大纲等信息，对这段剧情进行完善和扩展。

要求：
1. 仔细分析用户的剧情描述，理解核心情节走向和意图
2. 结合前文已有内容，确保剧情衔接自然、前后一致
3. 根据人物设定，确保角色行为符合其性格特征和人物弧线
4. 参考大纲结构，保持整体故事节奏和方向
5. 补充必要的场景描写、人物对话、心理活动和细节
6. 处理好伏笔和悬念的设置
7. 输出完善后的完整剧情内容，可直接用于写作
...
命运的两端，即将交汇。""",
    ...
    "plot_enhance": [
        """根据您提供的剧情描述，结合前文内容和人物设定，以下是完善后的剧情：

　　夜幕低垂，陆远独自站在星际港口的观景台上，远处的星光在他瞳孔中折射出细碎的光芒。三天前那封来自深空的加密信息，彻底打破了他在渔村平静的生活。

　　"父亲还活着。"他在心里默念着这四个字，手指无意识地攥紧了口袋里那枚老旧的星际导航仪。这是父亲失踪前留给他的唯一遗物，而如今，它竟然开始发出微弱的脉冲信号。

　　身后传来轻微的脚步声，陆远没有回头。他知道是苏晴——从小一起长大的伙伴，也是唯一知道他秘密的人。

　　"你真的决定了？"苏晴的声音里带着难以掩饰的担忧，"那片星域已经被联邦列为禁区整整十年了。"

　　"正因为如此，我才更要去。"陆远终于转过身，目光坚定，"联邦封锁那片星域的时间，恰好和父亲失踪的时间吻合。这不是巧合。"

　　苏晴沉默了片刻，从背包里取出一个小型全息投影仪："我查了联邦的旧档案，那片星域在封锁前，曾经有一支科考队在那里发现了……"她压低了声音，"一座不属于任何已知文明的遗迹。"
